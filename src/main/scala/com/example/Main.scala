...
object Transaction {
  ...
  case object Running extends TransactionState {
    def identifier: String = "running"
  }

  case object Done extends TransactionState {
    def identifier: String = "done"
  }

  case object Failed extends TransactionState {
    def identifier: String = "failed"
  }
...
class Transaction(id: String) extends PersistentFSM[TransactionState, TransactionPayload, DomainEvent] {
  ...
  when(Running) {
    case Event(CompleteTransaction, RunningTransaction(invoice)) => goto(Done) applying TransactionCompleted(invoice, time) andThen (_ => stop())
  ...
  def applyEvent(event: DomainEvent, oldData: TransactionPayload): TransactionPayload = {
    val newData = (event, oldData) match {
      ...
      case (event: TransactionRejected, state: RunningTransaction) => RejectedTransaction(state.invoice, event.reason, time)
